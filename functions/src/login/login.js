...
module.exports = async (req, res) => {
    ...
    if (temp[0].length > 0) {
        ...
        if (token_notificacao != null && token_notificacao.length > 0) {
            await db.query(`
            CALL PCD_CANTAI_ALTERACAO_USUARIOS_TOKEN_NOTIFICACAO_APP(${temp[0][0].ID_USUARIO}, '${token_notificacao}')
         `);
        }
    ...
    // Inserir Usuário
    return db.query(`
          CALL PCD_CANTAI_INCLUSAO_USUARIO_APP('${nome}', '${sobrenome}', '${email}','', '${imagem}', '${token_autenticacao}', '${token_notificacao}')
